...
| DVB SI basics | Broadcast TS often carries EIT p/f, SDT, NIT, and richer descriptors. | Generated synthetic TS carries minimal EIT p/f on PID `0x0012`, SDT actual on PID `0x0011`, and NIT actual on PID `0x0010`. Sample media profiles and `SATIP_LAB_TS_PATH` are not rewritten. | Partial | Use for basic SI parser fallback tests; validate full DVB SI behavior against real streams. |
